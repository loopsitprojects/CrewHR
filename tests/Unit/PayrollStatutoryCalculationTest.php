...
class PayrollStatutoryCalculationTest extends TestCase
{
    ...
    public function test_custom_epf_and_etf_rates_from_settings()
    {
        \Illuminate\Support\Facades\DB::table('settings')->insert([
            ['key' => 'epf_employee_rate', 'value' => '10.0'],
            ['key' => 'epf_employer_rate', 'value' => '15.0'],
            ['key' => 'etf_employer_rate', 'value' => '5.0'],
        ]);

        $totalForEpf = 100000.0;
        $statutory = $this->calcService->calculateEpfEtf($totalForEpf);

        $this->assertEquals(10000.0, $statutory['epf_employee']); // 10%
        $this->assertEquals(15000.0, $statutory['epf_employer']); // 15%
        $this->assertEquals(5000.0, $statutory['etf_employer']);  // 5%
        $this->assertEquals(25000.0, $statutory['total_epf']);    // 25%
    }
    ...
    public function test_double_entry_accounting_journal_balances_with_zero_variance()
    {
        ...
        $deptDirect = Department::create(['name' => 'IT / Creative', 'description' => 'Direct Production']);
        $deptIndirect = Department::create(['name' => 'Corporate HR', 'description' => 'Administration']);

        $desigDirect = Designation::create(['name' => 'Senior Engineer', 'department_id' => $deptDirect->id]);
        $desigIndirect = Designation::create(['name' => 'HR Executive', 'department_id' => $deptIndirect->id]);

        $empDirect = Employee::create([
            'user_id' => $user1->id,
            'department_id' => $deptDirect->id,
            'designation_id' => $desigDirect->id,
        ...
        $empIndirect = Employee::create([
            'user_id' => $user2->id,
            'department_id' => $deptIndirect->id,
            'designation_id' => $desigIndirect->id,
